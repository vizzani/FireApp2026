...
evac: [
{ id:‘evac_01’, desc:‘Verifica centrale EVAC e alimentazioni (rete + batteria)’, norm:‘UNI 11988:2025 - p.8.1 - UNI EN 54-16 - p.6.1’, freq:‘mensile’ },
{ id:‘evac_02’, desc:‘Test messaggi preregistrati di evacuazione zona per zona’, norm:‘UNI 11988:2025 - p.8.2 - UNI EN 54-16 - p.6.2’, freq:‘mensile’ },
{ id:‘evac_03’, desc:‘Verifica amplificatori e livelli di uscita (dBSPL)’, norm:‘UNI 11988:2025 - p.8.3 - UNI EN 54-16 - p.6.3’, freq:‘semestrale’ },
{ id:‘evac_04’, desc:‘Test microfono di emergenza e priorita messaggi’, norm:‘UNI 11988:2025 - p.8.4 - UNI EN 54-16 - p.6.4’, freq:‘semestrale’ },
{ id:‘evac_05’, desc:‘Misura intelligibilita del parlato STI/RASTI per zona (soglia >= 0.50)’, norm:‘UNI 11988:2025 - p.9.1 - UNI EN 54-16 - p.7’, freq:‘annuale’ },
{ id:‘evac_06’, desc:‘Verifica diffusori: integrita fisica e copertura acustica’, norm:‘UNI 11988:2025 - p.8.5 - UNI ISO 7240-19 - p.8.1’, freq:‘semestrale’ },
{ id:‘evac_07’, desc:‘Controllo segnalazione guasti alla centrale’, norm:‘UNI 11988:2025 - p.8.6 - UNI EN 54-16 - p.6.5’, freq:‘semestrale’ },
{ id:‘evac_08’, desc:‘Verifica autonomia batterie (min 24h standby + 30min allarme)’, norm:‘UNI 11988:2025 - p.8.7 - UNI EN 54-16 - p.6.6’, freq:‘semestrale’ },
{ id:‘evac_09’, desc:‘Verifica integrazione e interfaccia con impianto IRAI: comando/controllo bidirezionale e sequenze di attivazione’, norm:‘UNI 11988:2025 - p.7.3 - UNI 9795 - p.5.9’, freq:‘semestrale’ },
{ id:‘evac_10’, desc:‘Controllo registro degli eventi e log storico: allarmi, guasti, test e interventi di manutenzione’, norm:‘UNI 11988:2025 - p.9.2 - UNI 11224:2011 - p.5.2.5’, freq:‘semestrale’ },
{ id:‘evac_11’, desc:‘Verifica documentazione as-built aggiornata: planimetrie zone EVAC, schema a blocchi e relazione tecnica’, norm:‘UNI 11988:2025 - p.6 - D.M. 37/2008 - art.7’, freq:‘annuale’ },
],
sprinkler: [
...
showModal(`<div class="modal-handle"></div> <div class="modal-title">Account</div> <p style="font-size:14px;color:var(--gray-600);margin-bottom:6px"> ${state.profile?.full_name || state.user?.email}<br> <span style="font-size:12px">${state.org?.name || 'Organizzazione non configurata'}</span> </p> <div style="margin-bottom:20px"> <span style="font-size:11px;font-weight:700;padding:3px 10px;border-radius:99px;background:${planStyle.split(';color:')[0]};color:${planStyle.split(';color:')[1]}">Piano ${planLabel}</span> </div> ${isAdmin() ? '<button class="btn-secondary" style="width:100%;margin-bottom:8px" onclick="closeModal();showOrgSettingsModal()">Impostazioni azienda</button>' : ''} <button class="btn-secondary" style="width:100%;margin-bottom:8px" onclick="closeModal();showAboutModal()">Informazioni su FireApp</button> <button class="btn-danger" style="width:100%" onclick="signOut()">Esci</button>`);
});
...
showModal(`<div class="modal-handle"></div> <div class="modal-title">Password dimenticata</div> <p style="font-size:14px;color:var(--gray-600);margin-bottom:18px">Inserisci la tua email. Ti invieremo un link per reimpostare la password.</p> <div class="form-field"> <label>Email</label> <input id="reset-email" type="email" placeholder="tecnico@azienda.it"> </div> <div id="reset-msg" style="font-size:13px;color:var(--green-600);margin-bottom:12px;display:none"></div> <button class="btn-primary" onclick="sendResetPassword()">Invia link di reset</button> <button class="btn-outline" onclick="closeModal()">Annulla</button>`);
}
...
showModal(`<div class="modal-handle"></div> <div class="modal-title">Imposta nuova password</div> <p style="font-size:14px;color:var(--gray-600);margin-bottom:18px">Inserisci la nuova password per il tuo account.</p> <div class="form-field"> <label>Nuova password</label> <input id="new-pwd" type="password" placeholder="Minimo 8 caratteri"> </div> <div class="form-field"> <label>Conferma password</label> <input id="new-pwd2" type="password" placeholder="Ripeti la password"> </div> <div id="pwd-update-msg" style="font-size:13px;margin-bottom:10px;display:none"></div> <button class="btn-primary" onclick="updatePassword()">Salva nuova password</button>`);
}
...
‘<div class="equip-name"><strong>’ + (EQ_TYPE_LABELS[e.type] || e.type) + ‘</strong> × ’ + e.quantity + p6badge + ‘</div>’ +
...
showToast(‘showToast('Cliente aggiornato', 'success');
logAudit('client.update', 'client', clientId, { name });
...
logAudit(equipId ? ‘equipment.update’ : ‘equipment.create’, ‘equipment’, equipId, { type });
await openClient(clientId);
}
...
logAudit(‘equipment.delete’, ‘equipment’, equipId);
await openClient(clientId);
}
...
logAudit('client.create', 'client', null, { name });
await loadClienti();
}
...
logAudit(‘intervention.complete’, ‘intervention’, currentInterventionId);
navigate(‘verbali’);
loadVerbali();
}
...
logAudit(‘team.role_update’, ‘team’, null, { role: newRole });
loadTeam();
}
...
logAudit(‘team.remove’, ‘team’, null, { removed_name: name });
loadTeam();
}
...
return `<div class="agenda-deadline-item${isOver ? ' overdue' : ''}" onclick="closeModal && state.clients.length > 0 ? showNewInterventionModalForDeadline('${s.client_id}', '${s.equipment_type}') : null"> <div class="scad-dot ${isOver ? 'dot-red' : daysLeft <= 7 ? 'dot-amber' : 'dot-green'}"></div> <div style="flex:1;min-width:0"> <div style="font-size:13px;font-weight:600;color:var(--gray-900);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${s.clients?.name || '—'} · ${EQ[s.equipment_type] || s.equipment_type}</div> <div style="font-size:11px;color:var(--gray-500)">${capitalize(s.maintenance_type)} · ${dateStr}</div> </div> <div style="font-size:12px;font-weight:700;color:${isOver ? 'var(--red)' : daysLeft <= 7 ? 'var(--amber)' : 'var(--green-light)'};">${daysLabel}</div> </div>`;
};
...
`<div style="display:flex;align-items:center;gap:8px;margin-bottom:6px"> <label style="font-size:12px;color:var(--gray-600);width:140px;flex-shrink:0">${label}</label> <select style="flex:1;font-size:12px;padding:4px 8px;border:1px solid var(--gray-300);border-radius:6px" onchange="updateImportMapping('${field}', this.value)"> <option value="">— non importare —</option> ${(Object.keys(previewRows[0] || {})).map(h => `<option value=”${h}” ${_importMapping[field] === h ? ‘selected’ : ‘’}>${h}</option>` ).join('')} </select> </div>`
).join(’’);

// Preview table
const previewCols = Object.keys(previewRows[0] || {}).slice(0, 5);
const tableHtml = `<div style="overflow-x:auto;margin-bottom:12px"> <table class="import-preview-table"> <thead><tr>${previewCols.map(c =>`<th>${c}</th>`).join('')}</tr></thead> <tbody>${previewRows.map(row => `<tr>${previewCols.map(c => `<td>${row[c] || ''}</td>`).join(’’)}</tr>` ).join('')}</tbody> </table> </div>`;

area.innerHTML =
`<div style="background:var(--green-50,#f0fdf4);border-radius:8px;padding:10px 12px;margin-bottom:12px;font-size:13px;color:var(--green-700,#065f46)"> <strong>${total} righe trovate</strong> — prime 5 mostrate </div>` +
tableHtml +
...
// Risultato finale
if (resultArea) resultArea.innerHTML =
`<div style="background:${imported > 0 ? 'var(--green-50,#f0fdf4)' : '#fff5f5'};border-radius:8px;padding:12px 14px;margin-top:4px"> <div style="font-size:14px;font-weight:700;color:${imported > 0 ? 'var(--green-700,#065f46)' : 'var(--red)'}"> ${imported > 0 ? '✓ ' + imported + ' clienti importati' : '✗ Nessun cliente importato'} ${skipped > 0 ? ` · ${skipped} righe saltate (nome mancante o errore)` : ''} </div> </div>`;
