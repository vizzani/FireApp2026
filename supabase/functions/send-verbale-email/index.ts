...
//   RESEND_API_KEY     → ottieni da https://resend.com
//   RESEND_FROM_EMAIL  → mittente verificato su Resend, es. noreply@tuodominio.it
//   RESEND_FROM_NAME   → opzionale, default FireApp
...
<body style="margin:0;padding:0;background:#f3f4f6;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif">
  <div style="max-width:560px;margin:32px auto;background:white;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,.08)">
    ...
    const resendRes = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: `${Deno.env.get('RESEND_FROM_NAME') ?? 'FireApp'} <${Deno.env.get('RESEND_FROM_EMAIL') ?? 'noreply@fireapp.it'}>`,
        to: [recipient_email],
